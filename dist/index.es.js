import Te, { useState as fr } from "react";
var M = { exports: {} }, q = {};
/**
 * @license React
 * react-jsx-runtime.production.min.js
 *
 * Copyright (c) Facebook, Inc. and its affiliates.
 *
 * This source code is licensed under the MIT license found in the
 * LICENSE file in the root directory of this source tree.
 */
var Re;
function dr() {
  if (Re) return q;
  Re = 1;
  var y = Te, h = Symbol.for("react.element"), C = Symbol.for("react.fragment"), b = Object.prototype.hasOwnProperty, V = y.__SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED.ReactCurrentOwner, I = { key: !0, ref: !0, __self: !0, __source: !0 };
  function P(E, f, T) {
    var v, x = {}, j = null, U = null;
    T !== void 0 && (j = "" + T), f.key !== void 0 && (j = "" + f.key), f.ref !== void 0 && (U = f.ref);
    for (v in f) b.call(f, v) && !I.hasOwnProperty(v) && (x[v] = f[v]);
    if (E && E.defaultProps) for (v in f = E.defaultProps, f) x[v] === void 0 && (x[v] = f[v]);
    return { $$typeof: h, type: E, key: j, ref: U, props: x, _owner: V.current };
  }
  return q.Fragment = C, q.jsx = P, q.jsxs = P, q;
}
var S = {};
/**
 * @license React
 * react-jsx-runtime.development.js
 *
 * Copyright (c) Facebook, Inc. and its affiliates.
 *
 * This source code is licensed under the MIT license found in the
 * LICENSE file in the root directory of this source tree.
 */
var Oe;
function pr() {
  return Oe || (Oe = 1, process.env.NODE_ENV !== "production" && function() {
    var y = Te, h = Symbol.for("react.element"), C = Symbol.for("react.portal"), b = Symbol.for("react.fragment"), V = Symbol.for("react.strict_mode"), I = Symbol.for("react.profiler"), P = Symbol.for("react.provider"), E = Symbol.for("react.context"), f = Symbol.for("react.forward_ref"), T = Symbol.for("react.suspense"), v = Symbol.for("react.suspense_list"), x = Symbol.for("react.memo"), j = Symbol.for("react.lazy"), U = Symbol.for("react.offscreen"), _ = Symbol.iterator, ze = "@@iterator";
    function Be(e) {
      if (e === null || typeof e != "object")
        return null;
      var r = _ && e[_] || e[ze];
      return typeof r == "function" ? r : null;
    }
    var w = y.__SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED;
    function d(e) {
      {
        for (var r = arguments.length, t = new Array(r > 1 ? r - 1 : 0), o = 1; o < r; o++)
          t[o - 1] = arguments[o];
        Ce("error", e, t);
      }
    }
    function Ce(e, r, t) {
      {
        var o = w.ReactDebugCurrentFrame, l = o.getStackAddendum();
        l !== "" && (r += "%s", t = t.concat([l]));
        var s = t.map(function(i) {
          return String(i);
        });
        s.unshift("Warning: " + r), Function.prototype.apply.call(console[e], console, s);
      }
    }
    var Pe = !1, Xe = !1, De = !1, qe = !1, Se = !1, $;
    $ = Symbol.for("react.module.reference");
    function Ve(e) {
      return !!(typeof e == "string" || typeof e == "function" || e === b || e === I || Se || e === V || e === T || e === v || qe || e === U || Pe || Xe || De || typeof e == "object" && e !== null && (e.$$typeof === j || e.$$typeof === x || e.$$typeof === P || e.$$typeof === E || e.$$typeof === f || // This needs to include all possible module reference object
      // types supported by any Flight configuration anywhere since
      // we don't know which Flight build this will end up being used
      // with.
      e.$$typeof === $ || e.getModuleId !== void 0));
    }
    function Ie(e, r, t) {
      var o = e.displayName;
      if (o)
        return o;
      var l = r.displayName || r.name || "";
      return l !== "" ? t + "(" + l + ")" : t;
    }
    function ee(e) {
      return e.displayName || "Context";
    }
    function A(e) {
      if (e == null)
        return null;
      if (typeof e.tag == "number" && d("Received an unexpected object in getComponentNameFromType(). This is likely a bug in React. Please file an issue."), typeof e == "function")
        return e.displayName || e.name || null;
      if (typeof e == "string")
        return e;
      switch (e) {
        case b:
          return "Fragment";
        case C:
          return "Portal";
        case I:
          return "Profiler";
        case V:
          return "StrictMode";
        case T:
          return "Suspense";
        case v:
          return "SuspenseList";
      }
      if (typeof e == "object")
        switch (e.$$typeof) {
          case E:
            var r = e;
            return ee(r) + ".Consumer";
          case P:
            var t = e;
            return ee(t._context) + ".Provider";
          case f:
            return Ie(e, e.render, "ForwardRef");
          case x:
            var o = e.displayName || null;
            return o !== null ? o : A(e.type) || "Memo";
          case j: {
            var l = e, s = l._payload, i = l._init;
            try {
              return A(i(s));
            } catch {
              return null;
            }
          }
        }
      return null;
    }
    var R = Object.assign, X = 0, re, te, ne, oe, ae, ie, le;
    function se() {
    }
    se.__reactDisabledLog = !0;
    function Ue() {
      {
        if (X === 0) {
          re = console.log, te = console.info, ne = console.warn, oe = console.error, ae = console.group, ie = console.groupCollapsed, le = console.groupEnd;
          var e = {
            configurable: !0,
            enumerable: !0,
            value: se,
            writable: !0
          };
          Object.defineProperties(console, {
            info: e,
            log: e,
            warn: e,
            error: e,
            group: e,
            groupCollapsed: e,
            groupEnd: e
          });
        }
        X++;
      }
    }
    function ke() {
      {
        if (X--, X === 0) {
          var e = {
            configurable: !0,
            enumerable: !0,
            writable: !0
          };
          Object.defineProperties(console, {
            log: R({}, e, {
              value: re
            }),
            info: R({}, e, {
              value: te
            }),
            warn: R({}, e, {
              value: ne
            }),
            error: R({}, e, {
              value: oe
            }),
            group: R({}, e, {
              value: ae
            }),
            groupCollapsed: R({}, e, {
              value: ie
            }),
            groupEnd: R({}, e, {
              value: le
            })
          });
        }
        X < 0 && d("disabledDepth fell below zero. This is a bug in React. Please file an issue.");
      }
    }
    var Z = w.ReactCurrentDispatcher, N;
    function k(e, r, t) {
      {
        if (N === void 0)
          try {
            throw Error();
          } catch (l) {
            var o = l.stack.trim().match(/\n( *(at )?)/);
            N = o && o[1] || "";
          }
        return `
` + N + e;
      }
    }
    var W = !1, H;
    {
      var He = typeof WeakMap == "function" ? WeakMap : Map;
      H = new He();
    }
    function ue(e, r) {
      if (!e || W)
        return "";
      {
        var t = H.get(e);
        if (t !== void 0)
          return t;
      }
      var o;
      W = !0;
      var l = Error.prepareStackTrace;
      Error.prepareStackTrace = void 0;
      var s;
      s = Z.current, Z.current = null, Ue();
      try {
        if (r) {
          var i = function() {
            throw Error();
          };
          if (Object.defineProperty(i.prototype, "props", {
            set: function() {
              throw Error();
            }
          }), typeof Reflect == "object" && Reflect.construct) {
            try {
              Reflect.construct(i, []);
            } catch (g) {
              o = g;
            }
            Reflect.construct(e, [], i);
          } else {
            try {
              i.call();
            } catch (g) {
              o = g;
            }
            e.call(i.prototype);
          }
        } else {
          try {
            throw Error();
          } catch (g) {
            o = g;
          }
          e();
        }
      } catch (g) {
        if (g && o && typeof g.stack == "string") {
          for (var a = g.stack.split(`
`), p = o.stack.split(`
`), u = a.length - 1, c = p.length - 1; u >= 1 && c >= 0 && a[u] !== p[c]; )
            c--;
          for (; u >= 1 && c >= 0; u--, c--)
            if (a[u] !== p[c]) {
              if (u !== 1 || c !== 1)
                do
                  if (u--, c--, c < 0 || a[u] !== p[c]) {
                    var m = `
` + a[u].replace(" at new ", " at ");
                    return e.displayName && m.includes("<anonymous>") && (m = m.replace("<anonymous>", e.displayName)), typeof e == "function" && H.set(e, m), m;
                  }
                while (u >= 1 && c >= 0);
              break;
            }
        }
      } finally {
        W = !1, Z.current = s, ke(), Error.prepareStackTrace = l;
      }
      var B = e ? e.displayName || e.name : "", O = B ? k(B) : "";
      return typeof e == "function" && H.set(e, O), O;
    }
    function Qe(e, r, t) {
      return ue(e, !1);
    }
    function Ye(e) {
      var r = e.prototype;
      return !!(r && r.isReactComponent);
    }
    function Q(e, r, t) {
      if (e == null)
        return "";
      if (typeof e == "function")
        return ue(e, Ye(e));
      if (typeof e == "string")
        return k(e);
      switch (e) {
        case T:
          return k("Suspense");
        case v:
          return k("SuspenseList");
      }
      if (typeof e == "object")
        switch (e.$$typeof) {
          case f:
            return Qe(e.render);
          case x:
            return Q(e.type, r, t);
          case j: {
            var o = e, l = o._payload, s = o._init;
            try {
              return Q(s(l), r, t);
            } catch {
            }
          }
        }
      return "";
    }
    var D = Object.prototype.hasOwnProperty, ce = {}, fe = w.ReactDebugCurrentFrame;
    function Y(e) {
      if (e) {
        var r = e._owner, t = Q(e.type, e._source, r ? r.type : null);
        fe.setExtraStackFrame(t);
      } else
        fe.setExtraStackFrame(null);
    }
    function Ze(e, r, t, o, l) {
      {
        var s = Function.call.bind(D);
        for (var i in e)
          if (s(e, i)) {
            var a = void 0;
            try {
              if (typeof e[i] != "function") {
                var p = Error((o || "React class") + ": " + t + " type `" + i + "` is invalid; it must be a function, usually from the `prop-types` package, but received `" + typeof e[i] + "`.This often happens because of typos such as `PropTypes.function` instead of `PropTypes.func`.");
                throw p.name = "Invariant Violation", p;
              }
              a = e[i](r, i, o, t, null, "SECRET_DO_NOT_PASS_THIS_OR_YOU_WILL_BE_FIRED");
            } catch (u) {
              a = u;
            }
            a && !(a instanceof Error) && (Y(l), d("%s: type specification of %s `%s` is invalid; the type checker function must return `null` or an `Error` but returned a %s. You may have forgotten to pass an argument to the type checker creator (arrayOf, instanceOf, objectOf, oneOf, oneOfType, and shape all require an argument).", o || "React class", t, i, typeof a), Y(null)), a instanceof Error && !(a.message in ce) && (ce[a.message] = !0, Y(l), d("Failed %s type: %s", t, a.message), Y(null));
          }
      }
    }
    var Ne = Array.isArray;
    function F(e) {
      return Ne(e);
    }
    function We(e) {
      {
        var r = typeof Symbol == "function" && Symbol.toStringTag, t = r && e[Symbol.toStringTag] || e.constructor.name || "Object";
        return t;
      }
    }
    function Fe(e) {
      try {
        return de(e), !1;
      } catch {
        return !0;
      }
    }
    function de(e) {
      return "" + e;
    }
    function pe(e) {
      if (Fe(e))
        return d("The provided key is an unsupported type %s. This value must be coerced to a string before before using it here.", We(e)), de(e);
    }
    var ge = w.ReactCurrentOwner, Je = {
      key: !0,
      ref: !0,
      __self: !0,
      __source: !0
    }, ve, me;
    function Ke(e) {
      if (D.call(e, "ref")) {
        var r = Object.getOwnPropertyDescriptor(e, "ref").get;
        if (r && r.isReactWarning)
          return !1;
      }
      return e.ref !== void 0;
    }
    function Le(e) {
      if (D.call(e, "key")) {
        var r = Object.getOwnPropertyDescriptor(e, "key").get;
        if (r && r.isReactWarning)
          return !1;
      }
      return e.key !== void 0;
    }
    function Ge(e, r) {
      typeof e.ref == "string" && ge.current;
    }
    function Me(e, r) {
      {
        var t = function() {
          ve || (ve = !0, d("%s: `key` is not a prop. Trying to access it will result in `undefined` being returned. If you need to access the same value within the child component, you should pass it as a different prop. (https://reactjs.org/link/special-props)", r));
        };
        t.isReactWarning = !0, Object.defineProperty(e, "key", {
          get: t,
          configurable: !0
        });
      }
    }
    function _e(e, r) {
      {
        var t = function() {
          me || (me = !0, d("%s: `ref` is not a prop. Trying to access it will result in `undefined` being returned. If you need to access the same value within the child component, you should pass it as a different prop. (https://reactjs.org/link/special-props)", r));
        };
        t.isReactWarning = !0, Object.defineProperty(e, "ref", {
          get: t,
          configurable: !0
        });
      }
    }
    var $e = function(e, r, t, o, l, s, i) {
      var a = {
        // This tag allows us to uniquely identify this as a React Element
        $$typeof: h,
        // Built-in properties that belong on the element
        type: e,
        key: r,
        ref: t,
        props: i,
        // Record the component responsible for creating this element.
        _owner: s
      };
      return a._store = {}, Object.defineProperty(a._store, "validated", {
        configurable: !1,
        enumerable: !1,
        writable: !0,
        value: !1
      }), Object.defineProperty(a, "_self", {
        configurable: !1,
        enumerable: !1,
        writable: !1,
        value: o
      }), Object.defineProperty(a, "_source", {
        configurable: !1,
        enumerable: !1,
        writable: !1,
        value: l
      }), Object.freeze && (Object.freeze(a.props), Object.freeze(a)), a;
    };
    function er(e, r, t, o, l) {
      {
        var s, i = {}, a = null, p = null;
        t !== void 0 && (pe(t), a = "" + t), Le(r) && (pe(r.key), a = "" + r.key), Ke(r) && (p = r.ref, Ge(r, l));
        for (s in r)
          D.call(r, s) && !Je.hasOwnProperty(s) && (i[s] = r[s]);
        if (e && e.defaultProps) {
          var u = e.defaultProps;
          for (s in u)
            i[s] === void 0 && (i[s] = u[s]);
        }
        if (a || p) {
          var c = typeof e == "function" ? e.displayName || e.name || "Unknown" : e;
          a && Me(i, c), p && _e(i, c);
        }
        return $e(e, a, p, l, o, ge.current, i);
      }
    }
    var J = w.ReactCurrentOwner, he = w.ReactDebugCurrentFrame;
    function z(e) {
      if (e) {
        var r = e._owner, t = Q(e.type, e._source, r ? r.type : null);
        he.setExtraStackFrame(t);
      } else
        he.setExtraStackFrame(null);
    }
    var K;
    K = !1;
    function L(e) {
      return typeof e == "object" && e !== null && e.$$typeof === h;
    }
    function xe() {
      {
        if (J.current) {
          var e = A(J.current.type);
          if (e)
            return `

Check the render method of \`` + e + "`.";
        }
        return "";
      }
    }
    function rr(e) {
      return "";
    }
    var be = {};
    function tr(e) {
      {
        var r = xe();
        if (!r) {
          var t = typeof e == "string" ? e : e.displayName || e.name;
          t && (r = `

Check the top-level render call using <` + t + ">.");
        }
        return r;
      }
    }
    function Ae(e, r) {
      {
        if (!e._store || e._store.validated || e.key != null)
          return;
        e._store.validated = !0;
        var t = tr(r);
        if (be[t])
          return;
        be[t] = !0;
        var o = "";
        e && e._owner && e._owner !== J.current && (o = " It was passed a child from " + A(e._owner.type) + "."), z(e), d('Each child in a list should have a unique "key" prop.%s%s See https://reactjs.org/link/warning-keys for more information.', t, o), z(null);
      }
    }
    function ye(e, r) {
      {
        if (typeof e != "object")
          return;
        if (F(e))
          for (var t = 0; t < e.length; t++) {
            var o = e[t];
            L(o) && Ae(o, r);
          }
        else if (L(e))
          e._store && (e._store.validated = !0);
        else if (e) {
          var l = Be(e);
          if (typeof l == "function" && l !== e.entries)
            for (var s = l.call(e), i; !(i = s.next()).done; )
              L(i.value) && Ae(i.value, r);
        }
      }
    }
    function nr(e) {
      {
        var r = e.type;
        if (r == null || typeof r == "string")
          return;
        var t;
        if (typeof r == "function")
          t = r.propTypes;
        else if (typeof r == "object" && (r.$$typeof === f || // Note: Memo only checks outer props here.
        // Inner props are checked in the reconciler.
        r.$$typeof === x))
          t = r.propTypes;
        else
          return;
        if (t) {
          var o = A(r);
          Ze(t, e.props, "prop", o, e);
        } else if (r.PropTypes !== void 0 && !K) {
          K = !0;
          var l = A(r);
          d("Component %s declared `PropTypes` instead of `propTypes`. Did you misspell the property assignment?", l || "Unknown");
        }
        typeof r.getDefaultProps == "function" && !r.getDefaultProps.isReactClassApproved && d("getDefaultProps is only used on classic React.createClass definitions. Use a static property named `defaultProps` instead.");
      }
    }
    function or(e) {
      {
        for (var r = Object.keys(e.props), t = 0; t < r.length; t++) {
          var o = r[t];
          if (o !== "children" && o !== "key") {
            z(e), d("Invalid prop `%s` supplied to `React.Fragment`. React.Fragment can only have `key` and `children` props.", o), z(null);
            break;
          }
        }
        e.ref !== null && (z(e), d("Invalid attribute `ref` supplied to `React.Fragment`."), z(null));
      }
    }
    var Ee = {};
    function je(e, r, t, o, l, s) {
      {
        var i = Ve(e);
        if (!i) {
          var a = "";
          (e === void 0 || typeof e == "object" && e !== null && Object.keys(e).length === 0) && (a += " You likely forgot to export your component from the file it's defined in, or you might have mixed up default and named imports.");
          var p = rr();
          p ? a += p : a += xe();
          var u;
          e === null ? u = "null" : F(e) ? u = "array" : e !== void 0 && e.$$typeof === h ? (u = "<" + (A(e.type) || "Unknown") + " />", a = " Did you accidentally export a JSX literal instead of a component?") : u = typeof e, d("React.jsx: type is invalid -- expected a string (for built-in components) or a class/function (for composite components) but got: %s.%s", u, a);
        }
        var c = er(e, r, t, l, s);
        if (c == null)
          return c;
        if (i) {
          var m = r.children;
          if (m !== void 0)
            if (o)
              if (F(m)) {
                for (var B = 0; B < m.length; B++)
                  ye(m[B], e);
                Object.freeze && Object.freeze(m);
              } else
                d("React.jsx: Static children should always be an array. You are likely explicitly calling React.jsxs or React.jsxDEV. Use the Babel transform instead.");
            else
              ye(m, e);
        }
        if (D.call(r, "key")) {
          var O = A(e), g = Object.keys(r).filter(function(cr) {
            return cr !== "key";
          }), G = g.length > 0 ? "{key: someKey, " + g.join(": ..., ") + ": ...}" : "{key: someKey}";
          if (!Ee[O + G]) {
            var ur = g.length > 0 ? "{" + g.join(": ..., ") + ": ...}" : "{}";
            d(`A props object containing a "key" prop is being spread into JSX:
  let props = %s;
  <%s {...props} />
React keys must be passed directly to JSX without using spread:
  let props = %s;
  <%s key={someKey} {...props} />`, G, O, ur, O), Ee[O + G] = !0;
          }
        }
        return e === b ? or(c) : nr(c), c;
      }
    }
    function ar(e, r, t) {
      return je(e, r, t, !0);
    }
    function ir(e, r, t) {
      return je(e, r, t, !1);
    }
    var lr = ir, sr = ar;
    S.Fragment = b, S.jsx = lr, S.jsxs = sr;
  }()), S;
}
process.env.NODE_ENV === "production" ? M.exports = dr() : M.exports = pr();
var n = M.exports;
const we = "data:image/svg+xml,%3csvg%20width='300'%20height='50'%20viewBox='0%200%20500%2084'%20fill='none'%20xmlns='http://www.w3.org/2000/svg'%20xmlns:xlink='http://www.w3.org/1999/xlink'%3e%3crect%20width='500'%20height='84'%20fill='url(%23pattern0_10_2)'/%3e%3cdefs%3e%3cpattern%20id='pattern0_10_2'%20patternContentUnits='objectBoundingBox'%20width='1'%20height='1'%3e%3cuse%20xlink:href='%23image0_10_2'%20transform='scale(0.002%200.0119048)'/%3e%3c/pattern%3e%3cimage%20id='image0_10_2'%20width='500'%20height='84'%20preserveAspectRatio='none'%20xlink:href='data:image/png;base64,'/%3e%3c/defs%3e%3c/svg%3e", gr = ({
  isOpen: y,
  onClose: h
}) => y ? /* @__PURE__ */ n.jsx(
  "div",
  {
    style: {
      position: "fixed",
      top: 0,
      left: 0,
      right: 0,
      bottom: 0,
      backgroundColor: "rgba(0, 0, 0, 0.7)",
      display: "flex",
      justifyContent: "center",
      alignItems: "center",
      zIndex: 1e3
    },
    children: /* @__PURE__ */ n.jsxs(
      "div",
      {
        style: {
          backgroundColor: "#101020",
          color: "#fff",
          padding: "30px",
          borderRadius: "10px",
          maxWidth: "760px",
          width: "90%",
          position: "relative",
          border: "1px solid #FFD32A"
        },
        children: [
          /* @__PURE__ */ n.jsx(
            "button",
            {
              onClick: h,
              style: {
                position: "absolute",
                top: "10px",
                right: "10px",
                backgroundColor: "transparent",
                border: "none",
                color: "#fff",
                fontSize: "20px",
                cursor: "pointer"
              },
              children: "✕"
            }
          ),
          /* @__PURE__ */ n.jsxs("div", { style: { textAlign: "center", marginBottom: "20px" }, children: [
            /* @__PURE__ */ n.jsx(
              "img",
              {
                src: we,
                alt: "Intuition Logo",
                style: { width: "120px", marginBottom: "10px" }
              }
            ),
            /* @__PURE__ */ n.jsx("h3", { style: { color: "#FFD32A", fontSize: "1em", margin: "10px 0" }, children: "MAKE A CLAIM ABOUT YOURSELF" })
          ] }),
          /* @__PURE__ */ n.jsx(
            "p",
            {
              style: {
                textAlign: "center",
                fontSize: "0.9em",
                marginBottom: "20px"
              },
              children: "Contribute and help to build the Boss Fighters ecosystem !"
            }
          ),
          /* @__PURE__ */ n.jsxs("div", { style: { marginBottom: "15px" }, children: [
            /* @__PURE__ */ n.jsx(
              "label",
              {
                style: {
                  display: "block",
                  marginBottom: "5px",
                  fontSize: "0.9em",
                  textAlign: "left"
                },
                children: "What is your Boss Fighter user ID ?"
              }
            ),
            /* @__PURE__ */ n.jsx(
              "input",
              {
                type: "text",
                placeholder: "YOUR-BOSS-FIGHTERS-IDENTIFICATOR",
                style: {
                  width: "100%",
                  padding: "8px",
                  backgroundColor: "#1e1e30",
                  border: "1px solid #333",
                  color: "#fff",
                  borderRadius: "4px"
                }
              }
            )
          ] }),
          /* @__PURE__ */ n.jsxs("div", { style: { marginBottom: "15px" }, children: [
            /* @__PURE__ */ n.jsx(
              "label",
              {
                style: {
                  display: "block",
                  marginBottom: "5px",
                  fontSize: "0.9em",
                  textAlign: "left"
                },
                children: "Are you member of a guild on Boss Fighters ?"
              }
            ),
            /* @__PURE__ */ n.jsxs(
              "select",
              {
                style: {
                  width: "100%",
                  padding: "8px",
                  backgroundColor: "#1e1e30",
                  border: "1px solid #333",
                  color: "#fff",
                  borderRadius: "4px"
                },
                children: [
                  /* @__PURE__ */ n.jsx("option", { value: "", children: "Select" }),
                  /* @__PURE__ */ n.jsx("option", { value: "yes", children: "Yes" }),
                  /* @__PURE__ */ n.jsx("option", { value: "no", children: "No" })
                ]
              }
            )
          ] }),
          /* @__PURE__ */ n.jsxs("div", { style: { marginBottom: "15px" }, children: [
            /* @__PURE__ */ n.jsx(
              "label",
              {
                style: {
                  display: "block",
                  marginBottom: "5px",
                  fontSize: "0.9em",
                  textAlign: "left"
                },
                children: "Are you a PC fighter player ?"
              }
            ),
            /* @__PURE__ */ n.jsxs(
              "select",
              {
                style: {
                  width: "100%",
                  padding: "8px",
                  backgroundColor: "#1e1e30",
                  border: "1px solid #333",
                  color: "#fff",
                  borderRadius: "4px"
                },
                children: [
                  /* @__PURE__ */ n.jsx("option", { value: "", children: "Select" }),
                  /* @__PURE__ */ n.jsx("option", { value: "yes", children: "Yes" }),
                  /* @__PURE__ */ n.jsx("option", { value: "no", children: "No" })
                ]
              }
            )
          ] }),
          /* @__PURE__ */ n.jsxs("div", { style: { marginBottom: "15px" }, children: [
            /* @__PURE__ */ n.jsx(
              "label",
              {
                style: {
                  display: "block",
                  marginBottom: "5px",
                  fontSize: "0.9em",
                  textAlign: "left"
                },
                children: "Are you a PC or VR Boss player ?"
              }
            ),
            /* @__PURE__ */ n.jsxs(
              "select",
              {
                style: {
                  width: "100%",
                  padding: "8px",
                  backgroundColor: "#1e1e30",
                  border: "1px solid #333",
                  color: "#fff",
                  borderRadius: "4px"
                },
                children: [
                  /* @__PURE__ */ n.jsx("option", { value: "", children: "Select" }),
                  /* @__PURE__ */ n.jsx("option", { value: "pc", children: "PC" }),
                  /* @__PURE__ */ n.jsx("option", { value: "vr", children: "VR" }),
                  /* @__PURE__ */ n.jsx("option", { value: "both", children: "Both" })
                ]
              }
            )
          ] }),
          /* @__PURE__ */ n.jsxs("div", { style: { marginBottom: "25px" }, children: [
            /* @__PURE__ */ n.jsx(
              "label",
              {
                style: {
                  display: "block",
                  marginBottom: "5px",
                  fontSize: "0.9em",
                  textAlign: "left"
                },
                children: "Add Intuition Agent code if you have one :"
              }
            ),
            /* @__PURE__ */ n.jsx(
              "input",
              {
                type: "text",
                placeholder: "CODE",
                style: {
                  width: "100%",
                  padding: "8px",
                  backgroundColor: "#1e1e30",
                  border: "1px solid #333",
                  color: "#fff",
                  borderRadius: "4px"
                }
              }
            )
          ] }),
          /* @__PURE__ */ n.jsx("div", { style: { textAlign: "center" }, children: /* @__PURE__ */ n.jsx(
            "button",
            {
              style: {
                padding: "8px 20px",
                backgroundColor: "#FFD32A",
                color: "#000",
                border: "none",
                borderRadius: "5px",
                cursor: "pointer",
                fontWeight: "bold"
              },
              children: "VALIDATE"
            }
          ) })
        ]
      }
    )
  }
) : null, mr = () => {
  const [y, h] = fr(!1), C = () => h(!0), b = () => h(!1);
  return /* @__PURE__ */ n.jsxs(
    "div",
    {
      style: {
        backgroundColor: "#101020",
        color: "#fff",
        padding: "20px",
        textAlign: "center",
        border: "4px solid #0078D4",
        borderRadius: "5px",
        display: "flex",
        flexDirection: "column",
        alignItems: "center",
        maxWidth: "960px",
        margin: "0 auto",
        position: "relative",
        overflow: "hidden"
      },
      children: [
        /* @__PURE__ */ n.jsx(
          "div",
          {
            style: {
              position: "absolute",
              top: "0",
              left: "0",
              width: "100%",
              height: "4px",
              backgroundColor: "#0078D4"
            }
          }
        ),
        /* @__PURE__ */ n.jsx(
          "div",
          {
            style: {
              position: "absolute",
              bottom: "0",
              left: "0",
              width: "100%",
              height: "4px",
              backgroundColor: "#0078D4"
            }
          }
        ),
        /* @__PURE__ */ n.jsx(
          "img",
          {
            src: we,
            alt: "Intuition Logo",
            style: { width: "200px", marginBottom: "10px", marginTop: "20px" }
          }
        ),
        /* @__PURE__ */ n.jsx(
          "h2",
          {
            style: {
              fontSize: "1.2em",
              margin: "0 0 20px 0",
              color: "#FFD32A"
              // Couleur orange pour le texte BOSS FIGHTERS
            },
            children: "BOSS FIGHTERS COMMUNITY PLAYER MAP"
          }
        ),
        /* @__PURE__ */ n.jsxs("div", { style: { maxWidth: "80%", margin: "0 auto" }, children: [
          /* @__PURE__ */ n.jsx("p", { style: { fontSize: "0.9em", lineHeight: "1.5" }, children: "At first, there was nothing. And then, suddenly, the whole community appeared !" }),
          /* @__PURE__ */ n.jsx("p", { style: { fontSize: "0.9em", lineHeight: "1.5" }, children: "Everything of which the Boss Fighters community would one day be composed, would be born in an instant." }),
          /* @__PURE__ */ n.jsx("p", { style: { fontSize: "0.9em", lineHeight: "1.5" }, children: "A single species of condensed matter, exploding in a vast universe." }),
          /* @__PURE__ */ n.jsx("p", { style: { fontSize: "0.9em", lineHeight: "1.5" }, children: "Although energy would neither be created nor destroyed, the interaction between these newly-created atoms would continue to create something beautiful..." }),
          /* @__PURE__ */ n.jsx("p", { style: { fontSize: "0.9em", lineHeight: "1.5" }, children: "What had been separate would become whole again. And what would be created in the process would be even more beautiful than what came before..." }),
          /* @__PURE__ */ n.jsx("p", { style: { fontSize: "0.9em", lineHeight: "1.5" }, children: "Our story begins with the atom. The cornerstone of our ecosystem." }),
          /* @__PURE__ */ n.jsx("p", { style: { fontSize: "0.9em", lineHeight: "1.5" }, children: 'And our "atoms" start with you !' }),
          /* @__PURE__ */ n.jsx("p", { style: { fontSize: "0.9em", lineHeight: "1.5" }, children: "Every contribution will help build our ecosystem and make it healthy..." })
        ] }),
        /* @__PURE__ */ n.jsxs(
          "div",
          {
            style: {
              border: "1px solid #FFD32A",
              borderRadius: "10px",
              padding: "15px",
              margin: "20px 0",
              display: "inline-block",
              backgroundColor: "rgba(0, 0, 0, 0.3)",
              width: "80%",
              maxWidth: "700px"
            },
            children: [
              /* @__PURE__ */ n.jsx(
                "p",
                {
                  style: { fontSize: "0.9em", margin: "0 0 10px 0", textAlign: "left" },
                  children: 'Claims in Intuition, also referred to as "Triples" structured in Semantic Triple format :'
                }
              ),
              /* @__PURE__ */ n.jsx(
                "p",
                {
                  style: { fontSize: "0.9em", margin: "0 0 10px 0", textAlign: "left" },
                  children: "[Subject] ⇒ [Predicate] ⇒ [Object] (For example, a triple could be : [SciFi] [is] [strong Boss])"
                }
              ),
              /* @__PURE__ */ n.jsx("p", { style: { fontSize: "0.9em", margin: "0", textAlign: "left" }, children: "This keeps our attestations tidy !" })
            ]
          }
        ),
        /* @__PURE__ */ n.jsx(
          "button",
          {
            onClick: C,
            style: {
              marginTop: "20px",
              marginBottom: "20px",
              padding: "10px 20px",
              backgroundColor: "#FFD32A",
              color: "#000",
              border: "none",
              cursor: "pointer",
              borderRadius: "5px",
              fontSize: "1em",
              fontWeight: "bold"
            },
            children: "CREATE YOUR PLAYER"
          }
        ),
        /* @__PURE__ */ n.jsx(gr, { isOpen: y, onClose: b })
      ]
    }
  );
}, hr = () => /* @__PURE__ */ n.jsx("div", { children: "Graph Vizualisation component!" });
export {
  hr as GraphComponent,
  mr as PlayerMapHome,
  gr as RegistrationForm,
  mr as default
};
